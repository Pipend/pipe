...
# ajax-keywords :: [DataSourceCue, String] -> p [String]
export ajax-keywords = (source-and-language) ->
...
# make-auto-completer :: (AceEditor -> Boolean) -> [DataSourceCue, String] -> ... -> p {on-query-changed :: String -> p AST, get-completions :: AceEditorMethod}
export make-auto-completer = (filter-function, [data-source-cue, transpilation-language], on-api-keywords-feteched, on-query-changed, get-completions) ->

    # get the keywords (name of tables, databases, query language related commands) from api
    {keywords}:api-keywords <- ajax-keywords [data-source-cue, transpilation-language] .then
...
export make-auto-completer-default = (filter-function, source-and-language) ->
