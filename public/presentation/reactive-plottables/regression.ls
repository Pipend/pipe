...
module.exports = ({{Plottable, xplot}:Reactive, d3}) -> new Reactive.Plottable do
    (x) -> x
    ({change, toggle, fx, dfx}:change_, meta, view, result, {iden, margin, x, y, size, y-range, y-axis, x-axis, tooltip}:options, continuation) !->
        ...
        svg = dview.select-all 'svg.regression' .data [result]
        ...
        circle = svg.select-all \circle
            .data result
                ..enter!
                    ..append \circle
                        ..on 'mouseover', -> fx 'highlight', iden it
                        ..on 'mouseout', -> dfx 'highlight', iden it
                ..attr "cx", x-scale . x
                ..attr "cy", y-scale . y
                ..attr "r", size-scale . size
                ..style "fill", (item) -> 
                    m = meta[iden item] ? {}
                    if m.highlight or m.select then 'red' else 'blue'
